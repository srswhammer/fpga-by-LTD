"""Step 4: turn ordinary single-digit images into model-ready inputs.

The public functions are:
    load_trained_model()       load and verify the float32 baseline checkpoint
    preprocess_digit_image()  normalize a path/PIL image/NumPy array to MNIST form
    predict_image()           run preprocessing and float32 software inference
"""
...
DEFAULT_CHECKPOINT = ROOT / "runs" / "baseline_v1" / "best_model.pt"
...
class PredictionResult:
    """One software prediction and everything needed to inspect it."""
...
def preprocess_digit_image(
    ...
    *,
    ...
    tensor = torch.from_numpy(normalized.copy()).to(torch.float32).div_(255.0)
...
def load_trained_model(checkpoint_path: str | Path = DEFAULT_CHECKPOINT) -> nn.Module:
    """Load the selected checkpoint and reject a mismatched model.py."""

    checkpoint_path = Path(checkpoint_path)
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
    expected_hash = checkpoint["config"]["model_sha256"]
    actual_hash = hashlib.sha256((ROOT / "model.py").read_bytes()).hexdigest()
    if actual_hash != expected_hash:
        raise RuntimeError("model.py differs from the definition used to train this checkpoint")
    model = DigitCNN()
...
def predict_image(
    ...
    *,
    ...
    """Preprocess one image and classify it with the float32 software model."""

    preprocessing = preprocess_digit_image(source, polarity=polarity)
    if model is None:
        model = load_trained_model(checkpoint_path)
    model.eval()
    with torch.inference_mode():
        score_tensor = model(preprocessing.tensor)
        probability_tensor = torch.softmax(score_tensor, dim=1)
    scores = score_tensor[0].cpu().numpy().copy()
    probabilities = probability_tensor[0].cpu().numpy().copy()
