"""单数字图片预处理：像素行为保持第一版不变，只依赖 NumPy/Pillow。"""
...
class PreprocessResult:
    """保留原灰度图、裁剪图、最终 uint8 图和元数据，供 C 检查每步处理。"""
...
def _to_grayscale_uint8(source: ImageSource) -> np.ndarray:
    """接收路径、PIL 图片或数组，输出独立的 H×W uint8 灰度图。"""
...
def preprocess_digit_image(
    ...
    *,
    ...
    """把单个完整数字统一为白字黑底的 28×28 图。

    估计背景 → 增强对比度 → 找边界 → 等比缩至 19×19 内 → 放入画布并按重心居中。
    输出 normalized_uint8 可直接传给整数模型，不要再除以 255 或 256。
    本函数不负责多数字检测；空白或对比度不足时抛出异常。
    """
